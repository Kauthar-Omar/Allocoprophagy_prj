#!/bin/bash
#$ -cwd 
#$ -N c_paired_align_bowtie_Rnor_6.0
...
DATA="/users/abaud/komar/P50/allocoprophagy/output/trimming/paired/"

# OUT: output folder for alignemnt 
OUT="/users/abaud/komar/P50/allocoprophagy/output/Rnor_6.0_alignments/c_paired"
...
bowtie2 -x $INDEX/Rnor_6.0 -1 $R1 -2 $R2 -S $OUT/${NAME}_Rnor_6.0.sam 2> $OUT/${NAME}_Rnor_6.0.stat
...
bowtie2 -x $INDEX/Rnor_6.0 -1 $R1 -2 $R2 > /dev/null 2> $OUT/2_${NAME}_Rnor_6.0.stat