#!/bin/bash
#$ -cwd 
#$ -N cutadapt_align_bowtie_Rnor_6.0
...
DATA="/users/abaud/komar/P50/allocoprophagy/output/trimmed_cutadapt"

# OUT: output folder for alignemnt 
OUT="/users/abaud/komar/P50/allocoprophagy/output/Rnor_6.0_alignments/cutadapt_align"

mkdir -p $OUT

# going through the list of "R1" files - i.e. one per sample - i.e. 1 per task
R1=$(ls $DATA/*_R1_trimmed.fq.gz | sort | sed -n ${SGE_TASK_ID}p)

# getting sample name
NAME=$(basename $R1 | sed '_R1_trimmed.fq.gz//')
...
bowtie2 --threads $NSLOTS -x $INDEX/Rnor_6.0 -1 $R1 -2 $R2 -S $OUT/${NAME}_Rnor_6.0.sam 2> $OUT/${NAME}_Rnor_6.0.stat
