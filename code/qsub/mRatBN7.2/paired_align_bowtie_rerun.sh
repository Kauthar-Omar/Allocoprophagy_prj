#!/bin/bash
#$ -cwd 
#$ -N rerun_for_failed_paired_align_bowtie_mRatBN7.2
...
#$ -pe smp 4
#$ -t 1-5
#$ -l virtual_free=20G,h_rt=16:00:00
#$ -q long-sl7

## Defining input/output

# INDEX: folder where bowtie2 index is, TODO change if changing reference genome, e.g. to mRat7
INDEX="/users/abaud/komar/P50/allocoprophagy/data/reference_genome"
...
R1=$(ls $DATA/001_L_16523AAD_*_R1_paired.fq.gz $DATA/003_L_16548AAD_*_R1_paired.fq.gz $DATA/006_W_01301AAE_*_R1_paired.fq.gz $DATA/008_W_01348AAE_*_R1_paired.fq.gz $DATA/009_L_01357AAE_*_R1_paired.fq.gz | sort | sed -n ${SGE_TASK_ID}p)
