...
COVERAGE="$OUT_DIR/mRatBN7.2/coverage"
mkdir -p $COVERAGE
TMP_DIR="$OUT_DIR/tmp"
 
BAM=$(ls $SORT_DIR/*_paired.bam  | sort | sed -n ${SGE_TASK_ID}p)
SAMPLE=$(basename $BAM | sed 's/_paired.bam//')
...
## 
echo $TMP_BAM $(ls $SORT_DIR/${SAMPLE}*_unpaired.bam) | tr ' ' '\n' > $LIST
## 
## # 4. coverage
## 
...
## # tab file - WG (nb: 1..20,M,X,Y)
samtools coverage -A -b $LIST -q0 -Q0 > $COVERAGE/${SAMPLE}_Q0.cov
## 
## ### (hist per chr {1..20} )
## 
